...
class Solution:
    def compareVersion(self, version1: str, version2: str) -> int:
        ver1 = list(map(lambda x: int(x), version1.split('.')))
        ver2 = list(map(lambda x: int(x), version2.split('.')))

        short = min(len(ver1), len(ver2))
        ...
        if len(ver1) > len(ver2):
            for i in range(short, len(ver1)):
                if ver1[i] > 0:
                    return 1
            return 0
        else:
            for i in range(short, len(ver2)):
                if ver2[i] > 0:
                    return -1
            return 0
