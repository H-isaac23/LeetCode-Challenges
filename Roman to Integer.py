...
# Instead, the number four is written as IV. Because the one is before the five we subtract it making four. The
...
class Solution:
    def romanToInt(self, s: str) -> int:
        ...
        i = 0
        while i < len(s):
            if i != len(s) - 1 and s[i] + s[i + 1] in numerals.keys():
                sums += numerals[s[i] + s[i + 1]]
                i += 2
            else:
                sums += numerals[s[i]]
                i += 1
